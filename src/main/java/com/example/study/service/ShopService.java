...
public class ShopService {
  ...
  public ShopService(
      ProductRepository productRepository, OrderRepository orderRepository, Clock clock) {
  ...
  /**
   * 購入処理。 @Transactional により在庫の減少と注文の記録が1つのトランザクションで実行される。
  ...
  public void purchase(Integer productId, Integer quantity) {
    // 1. 在庫を確認する
    Product product =
        productRepository.findById(productId).orElseThrow(() -> new RuntimeException("商品が見つかりません"));
